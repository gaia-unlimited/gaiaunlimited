...
def densproj_(h1data,typ='xy',normtyp='lognorm',cnt=False,cmap=None,cmapname='viridis',cbar=False,
	...
	if typ == 'xy':
		...
		while i < h1data.size:
			...
			indfull =  np.where( tmparray > 0 )[0]
			meanvals.append(np.median(h1data[i:i+bin_incr][indfull]))
		...
		h2 = sutil.hist_nd([ddum['pxgc'],ddum['pygc']],bins=[hbinsx,hbinsy],normed=False,range=[[xmin,xmax], [ymin,ymax]])
		...
		if plot_avgcounts :
			...
			if normtyp == '':	
				h2.imshow(meanvals,smooth=False,cmapname=cmap,vmin=0,vmax=1)#,vmin=1e+2,vmax=1e+05)			
			if normtyp == 'lims':	
		...
		for i in range(hbinsx):
			for j in range(hbinsz):
				vals.append((h1data.reshape(hbinsx,hbinsy,hbinsz)[i][:,j]).sum())
		...
		h2 = sutil.hist_nd([ddum['pxgc'],ddum['pzgc']],bins=[hbinsx,hbinsz],normed=False,range=[[xmin,xmax], [zmin,zmax]])
		...
		h2 = sutil.hist_nd([ddum['pygc'],ddum['pzgc']],bins=[hbinsy,hbinsz],normed=False,range=[[ymin,ymax], [zmin,zmax]])
