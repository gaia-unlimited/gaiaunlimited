...
def apogee_sf(apparentH: float,unreddenedJK: float, position: SkyCoord):
    ...
    apogee_frac_path = '../data/apogee_sampling_fractions.csv'
    ...
    allFieldCenterL = [ tApogeeSF['GLON'][ tApogeeSF['FIELD']==f ][0] for f in allFieldNames]
    allFieldCenterB = [ tApogeeSF['GLAT'][ tApogeeSF['FIELD']==f ][0] for f in allFieldNames]
    allFieldNames = np.array([ tApogeeSF['FIELD'][ tApogeeSF['FIELD']==f ][0] for f in allFieldNames])
    allFieldRadii = np.array([ tApogeeSF['RADIUS'][ tApogeeSF['FIELD']==f ][0] for f in allFieldNames])
    coord_centres = SkyCoord(l=allFieldCenterL*units.deg,b=allFieldCenterB*units.deg, frame='galactic')
    ...
    for i in range(len(apparentH)):
        ...
        if len(selectionFraction)==1:
            ...
            to_return.append( (sum(selectionFraction) - np.prod(selectionFraction)*0.5*sum(akFraction)) )
